...
public class FileDisk {
    ...
    private static final String TAG = FileDisk.class.getSimpleName();
	...
	public FileInputStream getInputStream(String url, String key) throws Exception {
		...
		if (file.exists()) {
			...
			Log.d( TAG ,"getInputStream(String key) not exist");
	...
	public OutputStream getOutputStream(String url, String key) throws Exception {
		...
		Log.d( TAG ,"getOutputStream(String key)" + filePath + File.separator + key + "." + getImageSuffix(url) + ".temp");
	...
	public void renameFile(String url, String key) {
		...
		if (file.exists() && file.length() != 0) {
            ...
            if (newFile.exists() && newFile.length() != file.length()) {
                Log.v( TAG ,String.format("原文件已存在不匹配，先删除目标文件，临时文件长度%s, 目标文件长度%s", file.length() + "", newFile.length() + ""));
                newFile.delete();
            }
